...
trait Compile[F[_], ES <: Tuple, CTS <: Tuple] {
...
object Compile {
  ...
  private def conformHttp(rep: Response, version: Version, options: Options): Response = {
    ...
    if options.includeDateHeader then {
      rep.headerMap.setUnsafe("Date", currentTime())
    }

    if options.includeServerHeader then {
  ...
  given emptyTupleTs[F[_]](using F: Applicative[F]): Compile[F, EmptyTuple, EmptyTuple] = new Compile[F, EmptyTuple, EmptyTuple] {
    def apply(es: EmptyTuple, opts: Options, ctx: Context): Endpoint.Compiled[F] =
      Endpoint.Compiled[F] { (req: Request) =>
        val rep = Response()

        if ctx.wouldAllow.nonEmpty && opts.enableMethodNotAllowed then {
  ...
  type IsNegotiable[C] = io.finch.internal.OrElse[C <:< Coproduct, DummyImplicit]

  given nonEmptyTupleTs[F[_], A, EH <: Endpoint[F, A], ET <: Tuple, CTH, CTT <: Tuple](using
    ...
    def apply(es: Endpoint[F, A] *: ET, opts: Options, ctx: Context): Endpoint.Compiled[F] = {
      val handler = if opts.enableUnsupportedMediaType then respond415.orElse(respond400) else respond400
      ...
      Endpoint.Compiled[F] { (req: Request) =>
        underlying(Input.fromRequest(req)) match {
          case EndpointResult.Matched(rem, trc, out) if rem.route.isEmpty =>
            val accept = if negotiateContent then req.accept.map(a => Accept.fromString(a)).toList else Nil
            out
              .flatMap { (oa: Output[A]) =>
                given tra: ToResponse.Aux[F, A, CTH] = ntrA(accept)
                given tre: ToResponse.Aux[F, Exception, CTH] = ntrE(accept)
                oa.toResponse[F, CTH].map(r => conformHttp(r, req.version, opts))
              }
              .attempt
              .map { e =>
                trc -> e
