...
object Output {
  ...
  final private[finch] case class Payload[A](
    ...
    protected def copy(status: Status, charset: Option[Charset], headers: Map[String, String], cookies: Seq[Cookie]): Output[A] =
  ...
  final private[finch] case class Failure(
    ...
    protected def copy(status: Status, charset: Option[Charset], headers: Map[String, String], cookies: Seq[Cookie]): Output[Nothing] =
  ...
  implicit class OutputOps[A](val o: Output[A]) extends AnyVal {
    ...
    private[finch] def convertToResponse(tr: (A, Charset) => Response, tre: (Exception, Charset) => Response): Response = {
      val rep = o match {
