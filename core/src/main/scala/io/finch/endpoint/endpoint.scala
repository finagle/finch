...
package object endpoint {
  ...
  private[finch] class FromInputStream[F[_]](stream: Resource[F, InputStream], blockingEc: ExecutionContext)(implicit F: Async[F]) extends Endpoint[F, Buf] {
    ...
    final def apply(input: Input): Endpoint.Result[F, Buf] =
      EndpointResult.Matched(
        input,
        Trace.empty,
        stream.use(s => F.evalOn(readLoop(Buf.Empty, s).map(buf => Output.payload(buf)), blockingEc))
      )
  }
