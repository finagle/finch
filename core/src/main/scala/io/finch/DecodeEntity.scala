...
trait HighPriorityDecode extends LowPriorityDecode {
  ...
  implicit val decodeInt: DecodeEntity[Int] = instance(
    s =>
      try {
        Right(s.toInt)
  ...
  implicit val decodeLong: DecodeEntity[Long] = instance(
    s =>
      try {
        Right(s.toLong)
      } catch { case e: Throwable => Left(e) }
  ...
  implicit val decodeFloat: DecodeEntity[Float] = instance(
    s =>
      try {
        Right(s.toFloat)
      } catch { case e: Throwable => Left(e) }
  ...
  implicit val decodeDouble: DecodeEntity[Double] = instance(
    s =>
      try {
        Right(s.toDouble)
      } catch { case e: Throwable => Left(e) }
  ...
  implicit val decodeBoolean: DecodeEntity[Boolean] = instance(
    s =>
      try {
        Right(s.toBoolean)
      } catch { case e: Throwable => Left(e) }
  ...
  implicit val decodeUUID: DecodeEntity[UUID] = instance(
    s =>
...
trait LowPriorityDecode {
  ...
  /**
    ...
    */
  implicit def decodeFromGeneric[A, H <: HList, E](
      implicit
      gen: Generic.Aux[A, H],
      ev: (E :: HNil) =:= H,
      de: DecodeEntity[E]
