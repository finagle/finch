...
""")
  ...
  protected[finch] def asyncResponseBuilder[A, CT <: String](
      writer: (A, Charset) => Buf
    )(implicit
      w: Witness.Aux[CT]
...
object ToResponse extends HighPriorityToResponseInstances {
  ...
  object FromCoproduct {
    ...
    implicit def cconsToResponse[L, R <: Coproduct, CT <: String](implicit trL: ToResponse.Aux[L, CT], fcR: Aux[R, CT]): Aux[L :+: R, CT] = instance {
      case (Inl(h), cs) => trL(h, cs)
      case (Inr(t), cs) => fcR(t, cs)
    }
  }

  implicit def coproductToResponse[C <: Coproduct, CT <: String](implicit fc: FromCoproduct.Aux[C, CT]): Aux[C, CT] = fc
}
