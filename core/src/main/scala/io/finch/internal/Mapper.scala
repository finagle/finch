...
private[finch] trait HighPriorityMapperConversions extends LowPriorityMapperConversions {
  ...
  /**
    ...
    */
  implicit def mapperFromOutputHFunction[F[_]: Monad, A, B, FN, OB](
      f: FN
    )(implicit
  ...
  /**
    * @group HighPriorityMapper
    */
  implicit def mapperFromResponseHFunction[F[_]: Monad, A, FN, R](
      f: FN
    )(implicit
      ftp: FnToProduct.Aux[FN, A => R],
      ev: R <:< Response
    ): Mapper.Aux[F, A, Response] =
    instance(_.mapOutput { value =>
...
object Mapper extends HighPriorityMapperConversions {
  implicit def mapperFromEffectOutputHFunction[F[_]: Monad, A, B, FN, FOB](
      f: FN
    )(implicit
  ...
  implicit def mapperFromFutureResponseHFunction[F[_]: Monad, A, FN, FR](
      f: FN
    )(implicit
      ftp: FnToProduct.Aux[FN, A => FR],
      ev: FR <:< F[Response]
    ): Mapper.Aux[F, A, Response] =
    instance(_.mapOutputAsync { value =>
