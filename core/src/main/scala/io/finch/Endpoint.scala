 ...
 * }}}
 *
 * `Endpoint`s are also composable in terms of or-else combinator (known as a "space invader" operator `:+:`) that takes two `Endpoint`s and returns a coproduct
 * `Endpoint`.
 *
 * {{{
 *   import io.finch._
...
trait Endpoint[F[_], A] {
   ...
   */
  final def map[B](fn: A => B)(using F: Monad[F]): Endpoint[F, B] =
    mapAsync(fn.andThen(F.pure))

  /**
   * Maps this endpoint to the given function `A => Future[B]`.
   */
  final def mapAsync[B](fn: A => F[B])(using Monad[F]): Endpoint[F, B] =
   ...
   */
  final def mapOutput[B](fn: A => Output[B])(using F: MonadError[F, Throwable]): Endpoint[F, B] =
    mapOutputAsync(a => F.catchNonFatal(fn(a)))

  /**
   * Maps this endpoint to the given function `A => Future[Output[B]]`.
   */
  final def mapOutputAsync[B](fn: A => F[Output[B]])(using Monad[F]): Endpoint[F, B] =
    new Endpoint[F, B] with (Output[A] => F[Output[B]]) {
  ...
  final def transformOutput[B](fn: F[Output[A]] => F[Output[B]]): Endpoint[F, B] =
    new Endpoint[F, B] {
      final def apply(input: Input): Endpoint.Result[F, B] = self(input) match {
        case EndpointResult.Matched(rem, trc, out: F[Output[A]]) =>
  ...
  final def transformF[B](fn: F[A] => F[B])(using Monad[F]): Endpoint[F, B] =
    transform(fn)

  /**
   * Transform this endpoint to the given function `F[A] => F[B]`
   */
  final def transform[B](fn: F[A] => F[B])(using F: Monad[F]): Endpoint[F, B] =
    new Endpoint[F, B] {
   ...
   */
  final def product[B](other: Endpoint[F, B])(using MonadError[F, Throwable]): Endpoint[F, (A, B)] =
   ...
   */
  final def productWith[B, O](other: Endpoint[F, B])(p: (A, B) => O)(using
   ...
   */
  final def ::[B](other: Endpoint[F, B])(using
      pa: PairAdjoin[B, A],
      F: MonadError[F, Throwable]
  ...
  final def coproduct[B >: A](other: Endpoint[F, B]): Endpoint[F, B] = new Endpoint[F, B] {
    final def apply(input: Input): Endpoint.Result[F, B] = self(input) match {
      case a @ EndpointResult.Matched(_, _, _) =>
        other(input) match {
          case b @ EndpointResult.Matched(_, _, _) =>
            if a.rem.route.length <= b.rem.route.length then a else b
          case _ => a
        }
      case a =>
        other(input) match {
   ...
   */
  final def :+:[B](that: Endpoint[F, B])(using
      a: Adjoin[B :+: A :+: CNil],
      F: MonadError[F, Throwable]
  ): Endpoint[F, a.Out] = {
   ...
   * Consider using [[io.finch.Bootstrap]] instead.
   */
  final def toServiceAs[CT <: String](using
      F: Effect[F],
   ...
   * Consider using [[io.finch.Bootstrap]] instead
   */
  final def compileAs[CT <: String](using
      F: MonadError[F, Throwable],
      tr: ToResponse.Aux[F, A, CT],
      tre: ToResponse.Aux[F, Exception, CT]
   ...
   */
  final def rescue(pf: PartialFunction[Throwable, F[Output[A]]])(using
   ...
   * Recovers from any exception occurred in this endpoint by creating a new endpoint that will handle any matching throwable from the underlying future.
   */
  final def handle(pf: PartialFunction[Throwable, Output[A]])(using
      F: ApplicativeError[F, Throwable]
   ...
   */
  final def should(rule: String)(predicate: A => Boolean)(using F: MonadError[F, Throwable]): Endpoint[F, A] =
    mapAsync(a =>
      if predicate(a) then F.pure(a)
   ...
   */
  final def shouldNot(rule: String)(predicate: A => Boolean)(using MonadError[F, Throwable]): Endpoint[F, A] =
   ...
   *   an endpoint that will return the value of this reader if it is valid. Otherwise the future fails with an [[Error.NotValid]] error.
   */
  final def should(rule: ValidationRule[A])(using MonadError[F, Throwable]): Endpoint[F, A] =
   ...
   *   an endpoint that will return the value of this reader if it is valid. Otherwise the future fails with a [[Error.NotValid]] error.
   */
  final def shouldNot(rule: ValidationRule[A])(using MonadError[F, Throwable]): Endpoint[F, A] =
   ...
   */
  final def attempt(using MonadError[F, Throwable]): Endpoint[F, Either[Throwable, A]] =
...
object Endpoint {
  ...
  extension [F[_], L <: Tuple](self: Endpoint[F, L]) {

    /**
     * Converts this endpoint to one that returns any type with this [[shapeless.HList]] as its representation.
     */
    def as[A](using mirror: Mirror.ProductOf[A], F: Monad[F]): Endpoint[F, A] = self.map(tuple => mirror.fromProduct(tuple))

  }

  /**
   * Extension that adds convenience methods to endpoint for optional values.
   */
  extension [F[_], A](self: Endpoint[F, Option[A]]) {

    /**
     * If endpoint is empty it will return provided default value.
     */
    def withDefault[B >: A](default: => B)(using Monad[F]): Endpoint[F, B] =
      self.map(_.getOrElse(default))

    /**
     * If endpoint is empty it will return provided alternative.
     */
    def orElse[B >: A](alternative: => Option[B])(using Monad[F]): Endpoint[F, Option[B]] =
  ...
  /*  implicit final class ValueEndpointOps[F[_], B](val self: Endpoint[F, B]) extends AnyVal {
      ...
      /**
   * Converts this endpoint to one that returns any type with `B *: EmptyTuple` as its representation.
   */
      def as[A](using gen: Generic.Aux[A, B *: EmptyTuple], F: Monad[F]): Endpoint[F, A] =
        self.map(value => gen.from(value *: EmptyTuple))
    }*/

  private trait EndpointSemigroupK[F[_]] extends SemigroupK[Endpoint[F, _]] {
    def combineK[A](x: Endpoint[F, A], y: Endpoint[F, A]): Endpoint[F, A] =
      x.coproduct(y)
  }

  private trait EndpointMonoidK[F[_]] extends EndpointSemigroupK[F] with MonoidK[Endpoint[F, _]] {
    def empty[A]: Endpoint[F, A] = Endpoint.empty[F, A]
  }

  private class EndpointFunctor[F[_]: Monad] extends Functor[Endpoint[F, _]] {
    def map[A, B](fa: Endpoint[F, A])(f: A => B): Endpoint[F, B] = fa.map(f)
  }

  private class EndpointApply[F[_]](using MonadError[F, Throwable]) extends EndpointFunctor[F] with Apply[Endpoint[F, _]] {
    def ap[A, B](ff: Endpoint[F, A => B])(fa: Endpoint[F, A]): Endpoint[F, B] =
      ff.productWith(fa)((f, a) => f(a))
  }

  private class EndpointApplicative[F[_]](using MonadError[F, Throwable]) extends EndpointApply[F] with Applicative[Endpoint[F, _]] {
    def pure[A](x: A): Endpoint[F, A] =
      Endpoint.const[F, A](x)
  }

  private class EndpointAlternative[F[_]](using MonadError[F, Throwable])
      extends EndpointApplicative[F]
      with EndpointMonoidK[F]
      with Alternative[Endpoint[F, _]]

  given endpointAlternative[F[_]](using MonadError[F, Throwable]): Alternative[Endpoint[F, _]] =
    new EndpointAlternative[F]

  given endpointApplicative[F[_]](using MonadError[F, Throwable]): Applicative[Endpoint[F, _]] =
    new EndpointApplicative[F]

  given endpointFunctor[F[_]: Monad]: Functor[Endpoint[F, _]] =
    new EndpointFunctor[F]

  given endpointMonoidK[F[_]]: MonoidK[Endpoint[F, _]] =
   ...
   */
  def zero[F[_]](using F: Applicative[F]): Endpoint[F, EmptyTuple] =
    new Endpoint[F, EmptyTuple] {
      final def apply(input: Input): Result[F, EmptyTuple] =
        EndpointResult.Matched(input, Trace.empty, F.pure(Output.EmptyTuple))
    }

  /**
   * Creates an [[Endpoint]] that always matches and returns a given value (evaluated eagerly).
   */
  def const[F[_], A](a: A)(using F: Applicative[F]): Endpoint[F, A] =
    new Endpoint[F, A] {
   ...
   * }}}
   */
  def lift[F[_], A](a: => A)(using F: Sync[F]): Endpoint[F, A] =
    new Endpoint[F, A] {
      final def apply(input: Input): Result[F, A] =
   ...
   */
  def liftAsync[F[_], A](fa: => F[A])(using F: Sync[F]): Endpoint[F, A] =
    new Endpoint[F, A] {
      final def apply(input: Input): Result[F, A] =
   ...
   */
  def liftOutput[F[_], A](oa: => Output[A])(using F: Sync[F]): Endpoint[F, A] =
    new Endpoint[F, A] {
      final def apply(input: Input): Result[F, A] =
   ...
   */
  def liftOutputAsync[F[_], A](foa: => F[Output[A]])(using F: Sync[F]): Endpoint[F, A] =
    new Endpoint[F, A] {
      final def apply(input: Input): Result[F, A] =
   ...
   *   [[fromFile]]
   */
  def fromInputStream[F[_]](stream: Resource[F, InputStream])(using
      F: Sync[F],
      S: ContextShift[F]
   ...
   *   [[fromInputStream]]
   */
  def fromFile[F[_]](file: File)(using
      F: Sync[F],
      S: ContextShift[F]
  ): Endpoint[F, Buf] =
    fromInputStream[F](
  ...
  /**
   * Creates an [[Endpoint]] that serves an asset (static content) from a Java classpath resource, located at `path`, as a static content. The returned endpoint
   * will only match `GET` requests with path identical to asset's.
   *
   ...
   */
  def classpathAsset[F[_]](path: String)(using
      F: Sync[F],
      S: ContextShift[F]
  ): Endpoint[F, Buf] = {
   ...
   * }}}
   */
  def filesystemAsset[F[_]](path: String)(using
      F: Sync[F],
      S: ContextShift[F]
  ): Endpoint[F, Buf] = {
    val asset = new Asset[F](path)
   ...
   */
  def root[F[_]](using F: Sync[F]): Endpoint[F, Request] =
   ...
   */
  def pathAny[F[_]](using F: Applicative[F]): Endpoint[F, EmptyTuple] =
    new Endpoint[F, EmptyTuple] {
      final def apply(input: Input): Result[F, EmptyTuple] =
        EndpointResult.Matched(
          input.withRoute(Nil),
   ...
   */
  def pathEmpty[F[_]](using F: Applicative[F]): Endpoint[F, EmptyTuple] =
    new Endpoint[F, EmptyTuple] {
      final def apply(input: Input): Result[F, EmptyTuple] =
        if input.route.isEmpty then EndpointResult.Matched(input, Trace.empty, F.pure(Output.EmptyTuple))
   ...
   */
  def path[F[_]: Sync](s: String): Endpoint[F, EmptyTuple] =
    new MatchPath[F](s)
  ...
  /**
   * An [[Endpoint]] that reads an optional request body represented as `CT` (`ContentType`) and interpreted as `A`, into an `Option`. The returned [[Endpoint]]
   * only matches non-chunked (non-streamed) requests.
   */
  def bodyOption[F[_]: Sync, A: ClassTag, CT](using D: Decode.Dispatchable[A, CT]): Endpoint[F, Option[A]] =
   ...
   */
  def body[F[_]: Sync, A: ClassTag, CT](using d: Decode.Dispatchable[A, CT]): Endpoint[F, A] =
   ...
   * }}}
   */
  def binaryBodyStream[F[_]: Sync, S[_[_], _]](using
      LR: LiftReader[S, F]
   ...
   * }}}
   */
  def stringBodyStream[F[_]: Sync, S[_[_], _]](using
      LR: LiftReader[S, F]
   ...
   * }}}
   */
  def bodyStream[F[_]: Sync, S[_[_], _], A, CT <: String](using
      LR: LiftReader[S, F],
   ...
   */
  def jsonBodyStream[F[_]: Sync, S[_[_], _], A](using
      LR: LiftReader[S, F],
   ...
   */
  def textBodyStream[F[_]: Sync, S[_[_], _], A](using
      LR: LiftReader[S, F],
  ...
  /**
   * An evaluating [[Endpoint]] that reads an optional (in a meaning that a resulting `Seq` may be empty) multi-value query-string param `name` from the request
   * into a `Seq`.
   */
  ...
  def coproductAll[F[_], A](endpoints: Endpoint[F, A]*): Endpoint[F, A] =
    if endpoints.isEmpty then empty else endpoints.reduce(_ coproduct _)
}
