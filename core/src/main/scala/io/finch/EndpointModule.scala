...
trait EndpointModule[F[_]] {
   ...
   */
  def zero(implicit F: Applicative[F]): Endpoint[F, EmptyTuple] =
    Endpoint.zero[F]
   ...
   */
  def pathAny(implicit F: Applicative[F]): Endpoint[F, EmptyTuple] =
    Endpoint.pathAny[F]

  /**
   * An alias for [[Endpoint.pathEmpty]].
   */
  def pathEmpty(implicit F: Applicative[F]): Endpoint[F, EmptyTuple] =
   ...
   *
   * @note
   *   This method is implicit such that an implicit conversion `String => Endpoint[F, EmptyTuple]` works.
   */
  implicit def path(s: String)(implicit F: Sync[F]): Endpoint[F, EmptyTuple] =
    Endpoint.path[F](s)
