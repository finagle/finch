...
class DivSpec extends AnyFlatSpec with Matchers {
  ...
  it should "work if the request is a put and the divisor is not 0" in {
    div(Input.post("/20/10")).awaitValueUnsafe(dispatcher) shouldBe Some(2)
  }
  it should "give back bad request if we divide by 0" in {
    div(Input.post("/20/0")).awaitOutputUnsafe(dispatcher).map(_.status) shouldBe Some(Status.BadRequest)
  }
  it should "give back nothing for other verbs" in {
    div(Input.get("/20/10")).awaitValueUnsafe(dispatcher) shouldBe None
  }
}
