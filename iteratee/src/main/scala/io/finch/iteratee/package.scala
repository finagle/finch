...
package object iteratee extends IterateeInstances {
  ...
  /**
    ...
    */
  def enumeratorBody[F[_]: Effect, A, CT <: String](
    implicit
  ...
  /**
    ...
    * an `Enumerator[Future, A]`. The returned [[Endpoint]] only matches chunked (streamed) requests.
    */
  def enumeratorJsonBody[F[_]: Effect, A](
    implicit
...
trait IterateeInstances extends LowPriorityInstances {
  ...
  implicit def enumeratorToJsonResponse[F[_]: Effect, A](
    implicit
    e: Encode.Aux[A, Application.Json],
    w: Witness.Aux[Application.Json]
  ): ToResponse.Aux[Enumerator[F, A], Application.Json] = {
    withCustomIteratee[F, A, Application.Json](
      writer => Iteratee.foreachM[F, Buf]((buf: Buf) => futureToEffect(writer.write(buf.concat(ToResponse.NewLine))))
    )
  }
}
...
trait LowPriorityInstances {
  ...
  protected def futureToEffect[F[_]: Effect, A](future: => Future[A]): F[A] = {
    Effect[F].async[A](cb => {
      future.onFailure(t => cb(Left(t))).onSuccess(b => cb(Right(b)))
    })
  }

  implicit def enumeratorToResponse[F[_]: Effect, A, CT <: String](
    implicit
    e: Encode.Aux[A, CT],
    w: Witness.Aux[CT]
  ...
  protected def withCustomIteratee[F[_]: Effect, A, CT <: String](iteratee: Writer[Buf] => Iteratee[F, Buf, Unit])(
    implicit
    e: Encode.Aux[A, CT],
    w: Witness.Aux[CT]
  ): ToResponse.Aux[Enumerator[F, A], CT] = {
