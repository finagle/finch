...
trait IterateeInstances extends LowPriorityInstances {
  ...
  implicit def enumeratorToJsonResponse[F[_]: Effect, A](
    ...
    withCustomIteratee[F, A, Application.Json](writer => Iteratee.foreachM[F, Buf]((buf: Buf) => futureToEffect(writer.write(buf.concat(ToResponse.NewLine)))))
}
...
trait LowPriorityInstances {
  ...
  protected def withCustomIteratee[F[_]: Effect, A, CT <: String](
      iteratee: Writer[Buf] => Iteratee[F, Buf, Unit]
    )(implicit
      e: Encode.Aux[A, CT],
      w: Witness.Aux[CT]
    ): ToResponse.Aux[Enumerator[F, A], CT] =
